...
- `test/attach-backpressure.test.ts` must first reproduce the old behavior without candidate-only CLI arguments, then prove that an attach process keeps draining at least 4 MiB while its stdout is paused. The real Broker + shared-PTY harness must retain two clients and one attach PID, keep `slowClients` unchanged and Broker output queues empty, recover the latest output and a post-resume input round trip, then exit through identity-bound Broker stop. The bounded-overflow case must retain the latest marker, emit the visible truncation warning and stay within the configured/RSS bound.
- `sharedTerminals.maxAttachOutputBytes=104857600` allows 100 MiB per native attach without preallocation. With the default `12` tasks and `4` clients, the configured pool can retain at most 4800 MiB (about 4.69 GiB); increasing task/client ceilings with the same buffer is rejected once the 5 GiB configured-pool aggregate gate would be exceeded.
